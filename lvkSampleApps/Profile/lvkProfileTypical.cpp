...
private:
    ...
    std::vector<std::uint32_t> MeshBlockId[NUM_MODELS][NUM_IMAGES];
    ...
    virtual void Loop() override
    {
                    ...
                    for(std::size_t j=0;j < LKV_PROF_SCENE_DIM;j++)
                    {
                        for(std::size_t k=0;k < LKV_PROF_SCENE_DIM;k++)
                        {
                            ...
                            if(!MeshInstancesCreated)
                            {
                                ...
                                MeshBlockId[MeshId][TextureId].push_back(BlockId);
                            }
                        }
                    }
                }
        ...
        if(ResourcesLoaded())
        {
            ...
            
            {
                ...
                {
                    TestRenderer.BindMeshGroup(CommandBuffer,MeshGroupData[i]);
                    
                    for(std::size_t j=0;j < NUM_IMAGES;j++)
                    {
                        TestRenderer.BindImageGroup(CommandBuffer,ImageGroupData[j]);
                        
                        for(std::size_t k=0;k < MeshBlockId[i][j].size();k++)
                        {
                            ...
                            for(int l = 0;l < MeshGroupData[i].MeshGroup.Meshes[0].IndexedSubMeshCount;l++)
                            {
